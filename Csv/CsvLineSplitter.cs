...
#endif
                bool unterminatedQuotedValue = false;
                if (p >= 0 && (unterminatedQuotedValue = IsUnterminatedQuotedValue(values[p].AsSpan(), options)) && !options.RemoveEmbeddedQuotes)
                {
                        values[p] = StringHelpers.Concat(values[p], separator.ToString(), value);
                }
                else
                {
                    if (unterminatedQuotedValue && options.RemoveEmbeddedQuotes)
                        values[p] = StringHelpers.RemoveQuotes(values[p], separator.ToString());
